...
#### 3.2.0: Themes (**BACKLOG**) - **AREA:** ‹no-area› - <nobr>***DREAMED** REQT-3cb9k1mg2d*</nobr>
 ...
 - **3.2.3: Styling hooks** (**BACKLOG**) - **stable class per element kind, plus state classes** The viewer MUST give the title, each row, gutter, change marker, content cell, inline-change token, line ending chip, and fold row a stable class name naming its kind, plus a class for its change or highlight state, so that apps and themes can style the viewer without depending on its element structure. <nobr>***DREAMED** REQT-3928hx46s3*</nobr>
